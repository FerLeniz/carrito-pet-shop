...
async function getApi(){
  ...
  copyArrayProducts = arrayProducts.map((product) => {
         ...
         if (localStorage.getItem('shoppingCart') && localStorage.getItem('copyArrayProducts')) {
...
function choosePageOfProducts() {
  if (cardsPharmacy) {
    drawTableProductsSelected( filterArrayProducts(copyArrayProducts, "Medicamento"), cardsPharmacy);
  } else if (cardsToy) {
    drawTableProductsSelected( filterArrayProducts(copyArrayProducts, "Juguete"), cardsToy);
  }
}
...
function drawTableProductsSelected(array, id) {
  array.forEach((product) => {
    ...
    newDiv.innerHTML = `
                <div class="card my-3 shadow rounded" style="width: 17rem;">
                    <div id='img${product._id}' class='altImg'>
                        <img  class="card-img-top d-block" src='${
                          product.imagen
                        }' alt="${product.nombre}">
                    </div>
                    ...
                    <div class="card-body">
                        ...
                        <p class='${product.stock < 6 ? "bg-danger" : ""} text-white altText'> ${product.stock < 6 ? "Ultimas unidades!!!" : ""}</p>
    ...
    document.getElementById(`desc${product._id}`).addEventListener("click", function showDescription() {
    ...
    document.getElementById(`${product._id}`).addEventListener("click", (e) => {
      ...
      arrayProducts.forEach((pro) => {
        if (product._id == pro._id && product.stock > 0 && product.quantityToShow < pro.stock) {
          ...
          //localStorage.setItem('shoppingCart',JSON.stringify(shoppingCart))
          addToast("toastGreen", toastGreencontent);
          saveScStorage();
        }
      });
      //count in NAV
  ...
  counterNav.textContent = shoppingCart.length == 0 || shoppingCart.length == undefined ? 0 : shoppingCart.length;
}
  ...
  if (shoppingCart.length === 0) {
    modalBody.textContent = "El carrito esta vacio";
     ...
     shoppingCart.forEach((prod) => {
          !noRepeatArr.includes(prod) ? noRepeatArr.push(prod) : "";
        });
    modalBody.innerHTML = "";
...
function emptyAllScInModal(id) {
  ...
  if (id === "toastRed") {
    addToast(id, toastRedContent);
  } else {
    addToast(id, toastGreencontent);
  }
}
